...
export function buildSale(
  ...
  const branchSummary = { id: `branch-${branch}`, slug: branch, name: branch === 'colombo' ? 'Colombo – Main Branch' : branch === 'kandy' ? 'Kandy Branch' : 'Galle Branch' };
  const cashierSummary = { id: `u-${cashierName.replace(/\s+/g, '-').toLowerCase()}`, name: cashierName, email: `${cashierName.toLowerCase().replace(/\s+/g, '.')}@nexpos.app`, role: 'CASHIER' };
...
export const cashiersByBranch: Record<string, string[]> = {
  colombo: ['Nadeesha Perera', 'Kasun Fernando', 'Anushka Weerasinghe'],
  kandy: ['Ishara Jayasuriya', 'Ruwan Silva'],
  galle: ['Dilhani Wickrama', 'Ruwan Silva']
};

const DAY_WEIGHT = [1.28, 1.34, 0.82, 0.9, 0.96, 1.06, 1.12];
...
function generateHistory(): { sales: Sale[]; nextSeq: number } {
  ...
  let seq = 10140;
  for (let d = 13; d >= 0; d--) {
    const weight = DAY_WEIGHT[(13 - d) % DAY_WEIGHT.length];
    branchIds.forEach((branch) => {
      const base = branch === 'colombo' ? 9 : branch === 'kandy' ? 7 : 5;
      const count = Math.max(2, Math.round((base + rand() * 3) * weight));
      const cashiers = cashiersByBranch[branch] ?? ['Nadeesha Perera'];
      for (let i = 0; i < count; i++) {
        const hour = 8 + Math.floor(rand() * 13);
        if (d === 0 && hour > 15) continue;
        ...
        const subtotal = lines.reduce((sum, l) => sum + l.unitPrice * l.quantity, 0);
        const discount = rand() < 0.14 ? Math.round((subtotal * (0.03 + rand() * 0.07)) / 10) * 10 : 0;
        const statusRoll = rand();
        const status: SaleStatus =
          statusRoll < 0.028 ? 'VOIDED' : statusRoll < 0.055 ? 'REFUNDED' : 'COMPLETED';
        const payRoll = rand();
        const payment: PaymentMethod = payRoll < 0.44 ? 'CASH' : payRoll < 0.88 ? 'CARD' : 'SPLIT';
        sales.push(
          buildSale(
            `SALE-${seq++}`,
...
export const reorderSuggestions = [
  {
    ...
    sku: 'DRY-AN-1000',
    suggested: 60,
    reason: 'Sales velocity up 30% this week'
  },
  {
    product: 'Maliban Chocolate Puff 200g',
    sku: 'SNK-MB-200',
    suggested: 48,
    reason: 'Below reorder threshold for 3 days'
  },
  {
    product: 'Kotmale Cheddar Cheese 200g',
    sku: 'DRY-KC-200',
    suggested: 24,
    reason: 'Out of stock — 11 missed sales'
  },
  {
    product: 'Sandwich Bread Loaf 450g',
    sku: 'BAK-SB-450',
    suggested: 80,
    reason: 'Daily staple, sells out by 4 PM'
  }
];
